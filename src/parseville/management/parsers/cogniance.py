        ...
        if vacancy_elements:
            for vacancy in vacancy_elements:
                ...
                if soup:
                    ...
                    if card:
                        title = card.find('strong', id_='job-position').text
                        city = card.find('div', id_='job-location').text
                        # todo possible swap city to 'location', wtf is dd in sorfserve parser?
                        # todo add requirements option?
                        lang = ''
                        print title, city, lang
                        comp = Company.objects.get(alias='cogniance')

                        vacancy_obj, created = Vacancy.objects_get_or_create(name=title, company=comp)
